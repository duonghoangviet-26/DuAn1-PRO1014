...
<head>
    ...
    <style>
        ...
        .card-schedule {
            border: none; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);
            background: white; margin-bottom: 20px; position: relative; overflow: hidden;
            transition: transform 0.2s ease-in-out, box-shadow 0.2s;
        }
        .card-schedule:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); }

        .card-schedule::before {
            content: ''; position: absolute; top: 0; left: 0; bottom: 0; width: 5px;
        }
        .status-ranh::before { background-color: #0dcaf0; }
        .status-ban::before { background-color: #ffc107; }
        .status-nghi::before { background-color: #198754; } 

        .date-col {
            min-width: 100px; text-align: center; border-right: 1px dashed #e0e0e0; padding-right: 20px;
        }
        .date-day { font-size: 2.2rem; font-weight: 800; line-height: 1; color: #333; }
        .date-month { font-size: 0.9rem; text-transform: uppercase; color: #888; font-weight: 600; letter-spacing: 1px; }
        .date-year { font-size: 0.85rem; color: #aaa; }

        .info-col { padding-left: 20px; flex-grow: 1; }
        .tour-title { font-size: 1.1rem; font-weight: 700; color: #085f63; margin-bottom: 5px; }
        .tour-meta { font-size: 0.9rem; color: #666; margin-bottom: 8px; display: flex; align-items: center; gap: 15px; }
        .tour-meta i { color: #888; }
        
        .note-box { 
            background-color: #fff8e1; border-left: 3px solid #ffc107; 
            padding: 8px 12px; font-size: 0.9rem; color: #555; border-radius: 4px; 
        ...
        .btn-custom:hover { transform: translateY(-1px); box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        
        .badge-status {
            position: absolute; top: 15px; right: 15px; 
            font-size: 0.75rem; padding: 5px 12px; border-radius: 20px; font-weight: 600;
        }
        .badge-ranh { background-color: #e0f7fa; color: #006064; }
        .badge-ban { background-color: #fff3cd; color: #856404; }
        .badge-nghi { background-color: #d1e7dd; color: #0f5132; }
        
        .nav-pills .nav-link {
            color: #555; font-weight: 600; border: 1px solid #ddd; margin-right: 8px; border-radius: 6px;
        }
        .nav-pills .nav-link.active {
            background-color: #085f63; color: white; border-color: #085f63;
        }
    </style>
</head>
...
>>>>>>> ae38597d14f77eccf4821ffde91c7db97ae09013
    ...
    <div class="content">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <button class="btn btn-outline-secondary btn-sm" onclick="window.print()"><i class="fas fa-print me-1"></i> In lịch</button>
        </div>

        <ul class="nav nav-pills mb-4" id="pills-tab">
            <li class="nav-item"><button class="nav-link active" onclick="filterSchedule('all', this)">Tất cả</button></li>
            <li class="nav-item"><button class="nav-link bg-white" onclick="filterSchedule('active', this)">Sắp tới / Đang đi</button></li>
            <li class="nav-item"><button class="nav-link bg-white" onclick="filterSchedule('completed', this)">Đã hoàn thành</button></li>
        </ul>
        ...
        <div class="schedule-list">
            ...
            <?php else: ?>
                <?php foreach ($ds_lich as $lich): 
                    ...
                    // Badge text & color
                    $badgeText = ($status == 'ranh') ? 'Sẵn sàng' : (($status == 'ban') ? 'Đang đi tour' : 'Hoàn thành');
                    $badgeClass = 'badge-' . $status;
                ?>
                
                <div class="card-schedule p-3 d-flex flex-column flex-md-row align-items-center <?= $statusClass ?>" data-status="<?= $status ?>">
                    ...
                    <div class="schedule-info ms-md-3 flex-grow-1 w-100">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <h5 class="mb-1">
                                    <?php 
                                        $icon = ($status == 'ban') ? 'fa-plane-departure text-warning' : (($status == 'nghi') ? 'fa-check-circle text-success' : 'fa-clock text-info');
                                    ?>
                        ...
                        <div class="d-flex flex-wrap gap-2 mt-3 pt-2 border-top">
                            <a href="index.php?act=hdv_schedule_detail&id=<?= $lich['MaLichLamViec'] ?>" class="btn btn-outline-primary btn-custom">
                                <i class="fas fa-info-circle"></i> Chi tiết
                            </a>
                            
                            <a href="index.php?act=hdv_quanlykhach&id=<?= $lich['MaLichLamViec'] ?>" class="btn btn-success btn-custom text-white">
                                <i class="fas fa-user-check"></i> Điểm danh
                            </a>

                            <a href="index.php?act=hdv_guest_list&id=<?= $lich['MaLichLamViec'] ?>" class="btn btn-info btn-custom text-white">
                                <i class="fas fa-users"></i> DS Khách
                            </a>
                            
                            <a href="index.php?act=hdv_vanhanh&id=<?= $lich['MaLichLamViec'] ?>" class="btn btn-outline-danger btn-custom">
