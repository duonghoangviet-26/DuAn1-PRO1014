        ...
        <div class="container-fluid">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">Quản lý thông tin khách hàng cá nhân và doanh nghiệp</p>
                </div>
                <a href="index.php?act=creatKH" class="btn btn-primary shadow-sm">
                            ...
                            <?php if (!empty($listKhachHang)): ?>
                                <?php $stt = 1; foreach ($listKhachHang as $kh): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="fw-bold text-dark"><?= htmlspecialchars($kh['HoTen']) ?></div>
                                        </td>
                                        <td>
                                            <div class="d-flex flex-column">
                                                <a href="tel:<?= $kh['SoDienThoai'] ?>" class="text-decoration-none text-dark fw-bold mb-1">
                                                    <i class="fas fa-phone-alt text-success me-2"></i><?= htmlspecialchars($kh['SoDienThoai']) ?>
                                                </a>
                                                <a href="mailto:<?= $kh['Email'] ?>" class="text-decoration-none text-muted small">
                                                    <i class="fas fa-envelope text-secondary me-2"></i><?= htmlspecialchars($kh['Email']) ?>
                                                </a>
                                            </div>
                                        </td>
                                        <td>
                                            <?php if ($kh['LoaiKhach'] == 'cong_ty'): ?>
                                                <span class="badge badge-type type-company">Công ty</span>
                                            <?php else: ?>
                                                <span class="badge badge-type type-personal">Cá nhân</span>
                                            <?php endif; ?>
                                        </td>
                                        <td>
                                            <span class="text-muted small">
                                                <?= htmlspecialchars($kh['TenCongTy'] ?? '---') ?>
                                            </span>
                                        </td>
